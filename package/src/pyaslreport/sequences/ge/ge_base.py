...
class GEBaseSequence(BaseSequence):
    ...
    def _extract_ge_common_metadata(self) -> dict:
        ...
        if dcm_tags.GE_ASSET_R_FACTOR in d:
            bids_ge["AssetRFactor"] = d.get(dcm_tags.GE_ASSET_R_FACTOR, None).value
        if dcm_tags.GE_EFFECTIVE_ECHO_SPACING in d:
            bids_ge["EffectiveEchoSpacing"] = d.get(dcm_tags.GE_EFFECTIVE_ECHO_SPACING, None).value
        if dcm_tags.GE_ACQUISITION_MATRIX in d:
            bids_ge["AcquisitionMatrix"] = d.get(dcm_tags.GE_ACQUISITION_MATRIX, None).value
        if dcm_tags.GE_NUMBER_OF_EXCITATIONS in d:
            bids_ge["TotalAcquiredPairs"] = d.get(dcm_tags.GE_NUMBER_OF_EXCITATIONS, None).value
        ...
        if dcm_tags.MR_ACQUISITION_TYPE in d:
            bids_ge["MRAcquisitionType"] = d.get(dcm_tags.MR_ACQUISITION_TYPE, None).value
        else:
            bids_ge["MRAcquisitionType"] = "3D"

        # PulseSequenceType default is spiral if not present
        if dcm_tags.MR_ACQUISITION_TYPE in d:
            bids_ge["PulseSequenceType"] = d.get(dcm_tags.MR_ACQUISITION_TYPE, None).value
        else:
